...
module.exports = {
  ...
  * oss图片地址加工
  *
  * @description 可给阿里云oss的图片url地址拼装x-oss-process参数，以达到图片处理的目的。对应文档：https://help.aliyun.com/document_detail/44686.html?spm=5176.208357.1107607.31.3bdc390fZfFTSZ
  *
  * @param {String} url - 图片地址
  * @param {String} type - 处理类型。
  * @param {Object} opts - 规则参数
  * @return {String} 结果文本
  */
  ossImgUrlProcess(url, type, opts) {
    const { ctx } = this;
    const { clientType, versionNumber } = ctx.query;
    ...
    // FIXME:这个是对于2.0版本以下安卓的兼容处理，如果后续有强制升级，可以删掉。
    if (versionNumber && !compareVersion(versionNumber, '2.0.0') && clientType === 'android') {
      return url;
    }

    const urlObj = URL.parse(url, true); // url对象
    // const isIosOrAndroid = [ 'ios', 'android' ].indexOf(clientType) > 0;

    let paramStr = 'image/' + type;
    for (const key in opts) {
      // const isLength = [ 'h', 'w', 'l', 's' ].indexOf(key) >= 0;

      // let value = opts[key];
      // if (isLength && isIosOrAndroid) {
      //   value = value * 2;
      // }
      paramStr += `,${key}_${opts[key]}`;
    }
    ...
    return URL.format({
      ...
      query: {
        ...urlObj.query,
        'x-oss-process': paramStr + '/format,jpg/interlace,1',
      },
    });
  },
